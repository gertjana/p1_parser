defmodule P1.Telegram do
  ...
  @doc false
  def to_struct!(list) do
    case to_struct(list) do
  ...
  @doc false
  def to_struct([:header, manufacturer, model]), do: {:ok, %Header{manufacturer: manufacturer, model: model}}

  def to_struct([:version, version]), do: {:ok, %Version{version: version}}

  def to_struct([:equipment_identifier, channel, identifier]), do: {:ok, %EquipmentIdentifier{channel: channel, identifier: identifier}}

  def to_struct([:timestamp, timestamp]), do: {:ok, %Timestamp{timestamp: timestamp}}

  def to_struct([:total_energy, :consume, :normal, {value, unit}]), do: {:ok, %TotalEnergy{direction: :consume, tariff: :normal, value: value, unit: unit}}
  def to_struct([:total_energy, :consume, :low, {value, unit}]),    do: {:ok, %TotalEnergy{direction: :consume, tariff: :low, value: value, unit: unit}}
  def to_struct([:total_energy, :produce, :normal, {value, unit}]), do: {:ok, %TotalEnergy{direction: :produce, tariff: :normal, value: value, unit: unit}}
  def to_struct([:total_energy, :produce, :low, {value, unit}]),    do: {:ok, %TotalEnergy{direction: :produce, tariff: :low, value: value, unit: unit}}

  def to_struct([:tariff_indicator, indicator]), do: {:ok, %TariffIndicator{indicator: indicator}}

  def to_struct([:active_power, :consume, {value, unit}]), do: {:ok, %ActivePower{direction: :consume, phase: :all, value: value, unit: unit}}
  def to_struct([:active_power, :produce, {value, unit}]), do: {:ok, %ActivePower{direction: :produce, phase: :all, value: value, unit: unit}}
  def to_struct([:active_power, :l1, :consume, {value, unit}]), do: {:ok, %ActivePower{direction: :consume, phase: :l1, value: value, unit: unit}}
  def to_struct([:active_power, :l1, :produce, {value, unit}]), do: {:ok, %ActivePower{direction: :produce, phase: :l1, value: value, unit: unit}}
  ...
  def to_struct([:long_failures_log, count, events]) do
    {:ok, %LongFailureLog{count: count, events: Enum.map(events,
      fn ev -> %LongFailure{timestamp: Enum.at(ev, 0), duration: elem(Enum.at(ev, 1), 0), unit: elem(Enum.at(ev, 1), 1)} end)}}
  end

  def to_struct([:voltage_swells, :l1, count]), do: {:ok, %VoltageSwells{phase: :l1, count: count}}
  def to_struct([:voltage_swells, :l2, count]), do: {:ok, %VoltageSwells{phase: :l2, count: count}}
  def to_struct([:voltage_swells, :l3, count]), do: {:ok, %VoltageSwells{phase: :l3, count: count}}

  def to_struct([:voltage_sags, :l1, count]), do: {:ok, %VoltageSags{phase: :l1, count: count}}
  def to_struct([:voltage_sags, :l2, count]), do: {:ok, %VoltageSags{phase: :l2, count: count}}
  def to_struct([:voltage_sags, :l3, count]), do: {:ok, %VoltageSags{phase: :l3, count: count}}

  def to_struct([:voltage, :l1, {value, unit}]), do: {:ok, %Voltage{phase: :l1, value: value, unit: unit}}
  def to_struct([:voltage, :l2, {value, unit}]), do: {:ok, %Voltage{phase: :l2, value: value, unit: unit}}
  def to_struct([:voltage, :l3, {value, unit}]), do: {:ok, %Voltage{phase: :l3, value: value, unit: unit}}

  def to_struct([:amperage, :l1, {value, unit}]), do: {:ok, %Amperage{phase: :l1, value: value, unit: unit}}
  def to_struct([:amperage, :l2, {value, unit}]), do: {:ok, %Amperage{phase: :l2, value: value, unit: unit}}
  def to_struct([:amperage, :l3, {value, unit}]), do: {:ok, %Amperage{phase: :l3, value: value, unit: unit}}

  def to_struct([:text_message, text]), do: {:ok, %TextMessage{text: text}}

  def to_struct([:message_code, code]), do: {:ok, %MessageCode{code: code}}

  def to_struct([:mbus_device_type, channel, type]), do: {:ok, %MbusDeviceType{channel: channel, type: type}}

  def to_struct([:mbus_equipment_identifier, channel, identifier]), do: {:ok, %EquipmentIdentifier{channel: channel, identifier: identifier}}

  def to_struct([:mbus_device_measurement, channel, timestamp, {value, unit}]), do: {:ok, %MbusDeviceMeasurement{channel: channel, timestamp: timestamp, value: value, unit: unit}}
